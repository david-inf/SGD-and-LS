...
\KwData{$w^0\in\R^{(p+1)}$, $M>1$, $k^\ast$, $\epsilon>0$, $\alpha_0$, $\beta_0$,\\
	$\gamma\in(0,1)$, $\delta\in(0,1)$, $a\in\R^+$, $\mathtt{opt}\in\set{0,1,2}$}
$k \gets 0$\;
\While{\stopping}{
	\shuffleDB\;
	$y_0 \gets w^k$\;
	...
	\For{\forcond}{
		...
		$\alpha\gets\mathtt{reset(}\alpha_{t-1},\alpha_0,a,M,N,t,\mathtt{opt)}/\beta$ see~\ref{alg:reset-step}\; 
		$q\gets0$\;
		\Repeat{$f_{i_t}(y_{t+1})\leq f_{i_t}(y_t)+\gamma\alpha\nabla f_{i_t}(y_{t-1})^Td_t$ \Or $q\geq q^\ast$}{
			$\alpha\gets\delta\alpha$\;
			$y_{t+1} \gets y_t+\alpha d_t$\;
			$q\gets q+1$\;
		}
		$\alpha_t\gets\alpha$\;
...
\KwIn{$\alpha$, $\alpha_0$, $a$, $M$, $N$, $t$, $\mathtt{opt}$}
...
%\vspace{0pt}
%\begin{algorithm}[H]
%\caption{\texttt{reset}}\label{alg:reset-step2}
%\KwIn{$\alpha$, $\alpha_0$, $a$,\\$M$, $N$, $t$, $\mathtt{opt}$}
%\uIf{$t=0$}{\KwRet{$\alpha_0$}}
%\uElseIf{$\mathtt{opt}=0$}{$\alpha\gets\alpha$}
%\uElseIf{$\mathtt{opt}=1$}{$\alpha\gets\alpha_0$}
...
%\vspace{0pt}
%\begin{algorithm}[H]
%\caption{\texttt{restart}}\label{alg:m-restart2}
%\KwIn{$\beta_0$,\\$\nabla f_{i_t}(y_t)$, $d_{t-1}$}
%$q\gets0$\;
%$d_t\gets-\bigl((1-\beta)\nabla f_{i_t}(y_t)+\beta d_{t-1}\bigr)$\;
%\If{$\nabla f_{i_t}(y_{t-1})^Td_t\geq 0$}{
...
%\vspace{0pt}
%\begin{algorithm}[H]
%\caption{\texttt{correction}}\label{alg:m-correction2}
%\KwIn{$\beta_0$,\\$\nabla f_{i_t}(y_t)$, $d_{t-1}$}
%$\beta\gets\beta_0$\;
%$q\gets0$\;
%\Repeat{$\nabla f_{i_t}(y_t)^Td_t<0$ \Or $q\geq q^\ast$}{
