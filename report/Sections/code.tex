...
\begin{algorithm}
\caption{Mini-batch Gradient Descent with fixed or decreasing step-size and momentum.}\label{alg:SGD-fix-decre-m}
\KwData{$w^0\in\R^{(p+1)}$, $M>1$, $k^\ast$, $\epsilon>0$, $\set{\alpha_k}$, $\set{\beta_k}$}
$k \gets 0$\;
\While{$\norma{\nabla\func(w^k)}>\epsilon\wedge k<k^\ast$}{
	\shuffleDB\;
	$y_0 \gets w^k$\;
	$d_{-1}\gets0$\;
	\For{\forcond}{
...
\While{$\norma{\nabla\func(w^k)}>\epsilon\wedge k<k^\ast$}{
	...
	\For{\forcond}{
		...
		%		$\alpha\gets\alpha_0$\;
		$q\gets0$\;
		\While{$f_{i_t}(y_{t+1})>f_{i_t}(y_t)+\gamma\alpha\nabla f_{i_t}(y_{t-1})^Td_t\wedge q<q^\ast$}{
