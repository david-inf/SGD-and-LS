...
\begin{figure}
\centering
\subfloat[][\emph{Log-loss. If $uv\gg0$ then the example is labelled correctly; if $uv\ll0$ then the label is the wrong one.}\label{subfig:log-loss}]%
{\begin{tikzpicture}
\begin{axis}[xlabel=$uv$,ylabel={$\ell(u,v)$},ytick={1,2,4},axis lines=middle,enlargelimits,width=0.45\textwidth]
\addplot[samples=200,blue,smooth] {ln(1+exp(-x))};
\end{axis}
