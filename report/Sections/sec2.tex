...
The direction is an expression involving the gradient, so we want to compute the gradient w.r.t. $z^t$ on the mini-batch $B_t$ whose indices are randomly chosen $i_t\subset\set{1,\dots,N}$. Knowing that $\nabla f_i(w^k)=x^{(i)}r_i+\lambda w^k$
...
z^{t+1}=z^t-\alpha_t\nabla f_{i_t}(z^t)
\end{equation}
so the direction is defined as $d_t=-\nabla f_{i_t}(z^t)$ that is the \emph{anti-gradient} evaluated on the considered mini-batch, we know that on average is a \emph{descent direction} so the objective function doesn't decrease necessarily at each step.
...
\end{itemize}
The first choice sees the same step-size between the epochs and so the iterations. The second choice changes the step-size at every epoch, while being constant between iterations, that particular form ensures the convergence. For this two algorithms the momentum term in algorithm~\ref{alg:SGD-variants} is set to $\beta_0=0$.
...
For this algorithm the momentum term is set to $\beta_0=0$.% in algorithm~\ref{alg:SGD-variants}.
...
\end{tikzpicture}
\end{center}
so if $d_0=0$ the direction will be $d_t=-(1-\beta_0)\nabla f_{i_t}(z^t)$ that is a descent direction, see algorithm~\vref{alg:m-restart}. The resulting algorithm is \texttt{MSL-SGDM-R}.
